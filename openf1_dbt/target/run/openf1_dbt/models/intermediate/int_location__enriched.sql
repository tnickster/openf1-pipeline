

  create or replace view `openf1-pipeline`.`intermediate`.`int_location__enriched`
  OPTIONS()
  as -- depends_on: `openf1-pipeline`.`staging`.`stg_openf1__location`

WITH location AS(
    SELECT
        *
    FROM
        `openf1-pipeline`.`staging`.`stg_openf1__location`
),

drivers AS(
    SELECT
        *
    FROM
    `openf1-pipeline`.`staging`.`stg_openf1__drivers`
),

location_enriched AS (
    SELECT
        location.*,
        drivers.full_name,
        drivers.name_acronym,
        drivers.team_name,
        drivers.team_colour,
        drivers.headshot_url
    FROM location
        LEFT JOIN drivers ON location.session_key = drivers.session_key AND location.driver_number = drivers.driver_number
)

SELECT * FROM location_enriched;

