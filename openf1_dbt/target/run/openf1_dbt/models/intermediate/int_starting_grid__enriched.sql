  ...
  create or replace view `openf1-pipeline`.`intermediate`.`int_starting_grid__enriched`
  OPTIONS()
  as WITH starting_grid AS(
    SELECT
        *
    FROM
        `openf1-pipeline`.`staging`.`stg_openf1__starting_grid`
),
drivers AS(
    SELECT
        *
